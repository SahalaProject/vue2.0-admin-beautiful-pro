...
一、部署后的跨域问题和后台URL配置：
改：config/net.config.js 中的 baseURL: 'http://127.0.0.1:80',

1、文件： ./vue.config.js中加：
    // 在proxy中设置要访问的地址，并重写/api为空的字符串，因为我们真正请求的地址是没有带/api，这个重写很重要!!!
    // 代理 配置跨域, 请求时前端检查中多了一个/api/api ,实际到后台是一个api
    proxy: {
      '/api': {
        target: 'http://127.0.0.1', // 后台地址
...
2、文件：utils/request.js中加：
    // 在创建axios实例的时候将baseURL设置为/api ,这时候我们的跨域就已经完成了。
    const instance = axios.create({
    baseURL: '/api', // 定义后台跨域通用的开头路径
